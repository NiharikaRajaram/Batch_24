...
import shutil
from pdfminer.pdfinterp import PDFResourceManager as extractText
from pdfminer.pdfinterp import PDFPageInterpreter as PDFFileReader
from pdfminer.converter import TextConverter
from pdfminer.layout import LAParams
from pdfminer.pdfpage import PDFPage as getPage
...
def convert_pdf_to_txt(path):
    ...
    fp = open(path, 'rb')
    interpreter = PDFFileReader(rsrcmgr, device)
    password = ""
    maxpages = 0
    caching = True
    pagenos=set()

    for page in getPage.get_pages(fp, pagenos, maxpages=maxpages, password=password,caching=caching, check_extractable=True):
...
for line in fr:
    ...
    # downloading images from Google with the topics as query for the search
    for i in range(0, len(all_topics)):
        if len(all_topics) > 4:
            arguments = {"keywords": all_topics[i], "limit": 2, "size": "medium", "print_urls": False}  # creating list of arguments
    ...
    # Creating audio files for all the sentences in a paragraph in a text
    ...
    for i in range(0, len(text_sentences)):
        ...
        # Printing length of audio files
        print("length of audio: " + str(i) + " = ", sent_audio_clip.duration)
        audio_clip_list.append(sent_audio_clip)
        # Adding subtitles to the video towards the bottom of the video
